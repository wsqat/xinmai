...
//$cid = $_GET["cid"] == null ? "1" : $_GET[""];


$title = stripslashes(trim($_POST['supplier']));
$title = strip_tags($title);
if(empty($title)){
    echo "<script language='javascript' type='text/javascript'>";  
    echo "alert('查询不为空，请重新查询！');";  
    echo "window.location.href='index.php'";  
    echo "</script>"; 
}
//echo $title;
$array = mysql_fetch_assoc(mysql_query("select * from xm_supplier 
    where username like BINARY '%".$title."%' "));
//LIKE BINARY '%FIND%
$id = $array['id'];
if(empty($id)){
    echo "<script language='javascript' type='text/javascript'>";  
        ...
        <div class="type1">
            <?php
                ...
                //for($index=1;$index<7;$index++){
                    $sql = "select * from xm_supplier
    where username like BINARY '%".$title."%' order by id desc  limit 8 ";
                    //echo $sql;
                    $result = mysql_query($sql);
                    if($result&&mysql_num_rows($result)>0){
                        $i = 1;
                        // if($index==1)
                        echo "<div class='type-header'><p>".$title."</p></div>";
                        // else
                        ...
                        while ($row = mysql_fetch_array($result)) {
                            ...
                            if($i<=8 && !empty($photo)){
                                $i++;
                                if($i%4!=0){
                                    echo "<div class='goods_1'><a id='img_link' href='shop.php?id=".$nid."'>
                <img alt='".$title."'  src='../upload_images/head_photo/".$photo."'></a></div>";    
                                }else{
                                    echo "<div class='goods_1 goods_4'><a id='img_link' href='shop.php?id=".$nid."'>
                <img alt='".$title."'  src='../upload_images/head_photo/".$photo."'></a></div>";    
                                }
