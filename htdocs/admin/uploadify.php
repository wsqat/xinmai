...
if (!empty($_FILES)) {
  ...
  //   mysql_query($query , $db) or die(mysql_error($db));
	...
	$imagenames[0] = date("Y-m-d h:m:s",time());
	...
	if (move_uploaded_file($tempFile, $path.$imagename)){
        ...
        if($_SESSION["user_role"]==1)
            $query = 'update xm_customer set header_photo="'.$imagename.'" where id='.$_SESSION["id"];
        elseif($_SESSION["user_role"]==2)
            $query = 'update xm_supplier set header_photo="'.$imagename.'" where id='.$_SESSION["id"];
