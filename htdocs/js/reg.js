$(document).ready(function (){
    ...
    $('#inlineCheckbox1').blur(function(){
        checkEmpty($('#inlineCheckbox1'));
    })
    ...
    $('#reg').click(function () {
        $.post("php/chk_code.php?act=char",{code:$('#yzm').val()},function(msg){
            if(regexEnum.name.test($('#name').val())==true&&regexEnum.password.test($('#password').val())&&($('#password').val()==$('#password2').val())&&regexEnum.phone.test($('#phone').val())&&regexEnum.email.test($('#email').val())) {
                if($("#inlineCheckbox1").prop('checked')==true){
                    //&&(regexEnum.qq.test($('#qq').val())||$('#qq').val()=='')
                    $('#reg').html('注册ing...');
                    //console.log('符合要求,可以注册（用于测试）,确定后跳转到未完工的页面');
                     alert('符合要求,可以注册,确定后跳转至首页');
                    //$('.form-horizontal').attr("action", "reg.php");
                    $('#regform').submit();

                    return true;
                }else{
                    alert('未选择同意并阅读<新麦用户注册协议>,不可以注册');
                    return false;    
                }
