...
if(!empty($_GET)){
    ...
    $sql = "select * from xm_blog where name= '$name' ";
    ...
    if($num >= 1){
        ...
        echo_message("您所搜索的用户还没有为自己添加个人传记,可以申请为其添加！",3);
    }
    //echo $name;
    //echo $des;
    $conne->close_conn();    
...
elseif(!empty($_POST)){
    $name = stripslashes(trim($_POST['search']));
    mysql_query("set names 'utf8'");    
    $sql = "select * from xm_blog where name= '$name' ";
    $num = $conne->getRowsNum($sql);
    if($num >= 1){
            ...
            <div class="down">
                ...
                <table id="tab1">
                    <tr><td><label>姓名:</label></td> 
                            <td><?php echo $name;if(empty($photo))
            $photo = "1.jpg";?></td></tr>
